...
public class TurnPID extends PIDCommand {
  private static SimpleMotorFeedforward m_feedforward = new SimpleMotorFeedforward(TurnPIDConstants.kS,
      TurnPIDConstants.kV, TurnPIDConstants.kA);
    ...
    super(
        ...
        output -> {
          drivetrain.arcadeDrive(0,
              output + m_feedforward.calculate(drivetrain.getHeading() + angleGoal),
              true);
        });

    getController().enableContinuousInput(-180, 180);
    getController() // Configure additional PID options by calling `getController` here.
